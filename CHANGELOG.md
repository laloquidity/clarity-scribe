...
- **Scribe can now transcribe an audio file, not just a microphone.** Until now the Local API only drove live dictation — start, stop, read history — so a recording you already had (a client call, a voice memo, an interview) had to go to a cloud service, which is exactly the wrong place for a confidential conversation. `POST /v1/audio/transcriptions` takes an uploaded file and returns `{"text": "…"}` on the same Parakeet/Whisper models that serve dictation, entirely on-device: the audio is never written to disk and never leaves the machine.
  ...
  It deliberately implements the **OpenAI audio-transcription contract**, which most of the transcription ecosystem has copied, so an app already written against that contract needs **no code at all** — base URL `http://127.0.0.1:5111/v1`, and the Scribe token pasted in as the API key. `response_format` supports `json` (default), `text` and `verbose_json`; `language` takes an ISO code; `model` is accepted and ignored, because the engine comes from Settings rather than from an HTTP request that could leave the app in a changed state.

  **Any format, any sample rate.** `.m4a` `.mp3` `.wav` `.webm` `.ogg` `.oga` `.opus` `.flac` `.mp4` `.mpga` `.aac` `.aiff` `.caf` are decoded by the Chromium media stack Electron already ships — which is why this cost zero new dependencies and no growth in the installer, where bundling FFmpeg would have added tens of megabytes and a per-platform binary to sign. Whatever arrives is resampled to 16 kHz and downmixed to mono natively, so an `.m4a` straight off a phone works unchanged, and long audio is split on Silero VAD speech boundaries exactly as a long dictation is. Uploads are capped at 25 MB — about a 45-minute call at mono 32–64 kbps — and 45 minutes of speech transcribes in about 52 seconds on an RTX 3090 (measured outside the app's window; the app itself runs somewhat slower).

  **Live dictation keeps priority.** A file job and a dictation would otherwise compete for the same GPU sessions, and the app's core promise is that the hotkey feels instant. So an upload that arrives mid-dictation is refused with `409` and a message saying to retry, rather than queued behind — and a second upload is refused the same way, since the endpoint transcribes one file at a time.

  **Errors are written to be read by a person.** Every failure comes back in the OpenAI envelope (`{"error":{"message":…}}`), because clients built against that contract show `message` verbatim to a user. Uploads are sniffed by magic number before any work happens, so sending the wrong file says "That file looks like a PDF document, not an audio recording" instead of a generic decode failure — and a name that disagrees with the bytes (AAC written under an `.mp3` name, which phones do constantly) is decoded by what it actually is. A too-large upload now answers `413` and *drains* the rest of the request instead of destroying the socket, so the caller reads the message explaining the limit rather than a connection reset.

  Scribe has to be **running** — it hosts the server, and its window is what holds the audio decoders — but it does not need focus and can sit minimized in the tray. The transcript goes through the same cleanup a dictation does (filler-word removal, Personal Dictionary replacement, Smart Formatting when enabled), with one deliberate exception: **Spoken Punctuation is not applied**, because it is a dictation command language and in a recorded conversation "period" is an ordinary word.

### 🐛 Fixes

- **Smart formatting no longer turns "first" into "1st" in ordinary sentences.** Every spoken ordinal was converted to a numeral, so the enumerating adverb that opens a sentence came out as a digit — "First, let's make sure everything is done" became "1st, let's…", and "it should actually write out first" became "…write out 1st". Idioms were hit the same way: "first of all", "at first", "second to none", "third party", "first name". No list of idioms could fix this, because the word is fine and it is the *numeral* that is wrong in prose. Ordinals below ten now stay spelled out, matching the rule this feature already applied to plain numbers (a standalone "five" was always left as a word) and ordinary English convention. Ten and above still convert ("the 21st century", "finished 11th"), dates are untouched ("january fifth" → "January 1"), addresses and numbered series still use numerals ("3rd floor", "5th Avenue", "4th birthday"), and two readings that a numeral would destroy are preserved: fractions ("a tenth of the budget") and the fixed phrase "the eleventh hour".
...
- **Command mode** — a second hotkey (default **F10**, configurable) records a spoken **command** instead of dictation: *"open my downloads folder"*, *"search for flights to Tokyo"*, *"show me my last three transcripts"*. The transcript is routed by a **local LLM** (llama.cpp + Gemma 4 E2B, auto-discovered; fully offline) to one of a curated tool set — type text, copy to clipboard, open apps/folders/URLs, web search, show transcripts — through a staged pipeline narrated live in the capsule: **listening → interpreting → proposal → executing → done**.
  - **Risk rulebook** (assessed on the actual arguments, not just the tool): the default is **just do it** — a spoken command is already an explicit instruction. Opening folders/apps/URLs, web searches, typing, copying: execute immediately. **Confirm** (proposal card, ↵/Esc, auto-cancel on no answer) is reserved for consequential actions — launching executable/script files (`.exe .bat .ps1 .msi …` — "open my downloads folder" is auto, "open setup.exe" confirms), and future tools that message, delete, or change system state. **Refuse** (explain, never run) is wired for future severe tiers (payments, credentials, bulk deletion).
